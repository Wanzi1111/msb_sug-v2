<template>
  <div class="app-container">
    ...
    <basics-form ref="form" :loading="loadings._operation_resource_updatePublish" :forms="forms" :actions="actions" @submitForm="submitForm" @changeForm="changeForm" />
  </div>
</template>

<script>
import BasicsForm from '@/components/BasicsForm'
import { updatePublish } from '@/api/advert/put'
import forms, { actions as formAct } from '../forms/edit'
...
export default {
  ...
  mounted() {
    const { bussinessName, advertName, enabled, startTime, endTime } = this.rowData
  ...
  methods: {
    ...
    submitForm(val) {
      ...
      const data = {
        ...
        advertIds: this.rowData.advertId,
        id: this.rowData.id
      }
      ...
      updatePublish(data).then(() => {
        this.$message.success('修改投放成功')
        this.close()
