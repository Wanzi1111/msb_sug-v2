<template>
  <div>
    <div class="title-container">
      <basics-title title="400客服投诉 [后台创建记录]" :actions="act" />
    </div>
    <div class="app-container">
      <basics-table
        ...
        :table="table"
        :loading="
          loadings._k8s_api_complaint_api_v2_complaint_batchComplaintList ||
            loadings._k8s_api_complaint_api_v2_complaint_exportBatchRecord
        "
        :list="list"
        :get-list="getList"
        :columns="columns"
...
export default {
  ...
  methods: {
    addSug() {
      this.$router.push({ name: 'createService', params: { type: '2' }})
      // this.info.title = '创建投诉'
      // this.dialogVisible = true
      // this.sugForm = userForm
    ...
    jump() {
      this.$router.push({ name: 'batchImport', params: { type: '2' }})
    },
    ...
    downLoadSuccessFn(row) {
      ...
      exportBatchRecord(params).then(res => {
        if (res.type === 'application/octet-stream') {
          createBlob(
            `投诉单成功导出数据-${moment().format('YYYY-MM-DD')}.xlsx`,
            res
          )
    ...
    downLoadErrorFn(row) {
      ...
      exportBatchRecord(params).then(res => {
        createBlob(
          `投诉单失败导出数据-${moment().format('YYYY-MM-DD')}.xlsx`,
          res
        )
        if (res.type === 'application/octet-stream') {
          Message.success('导出成功')
