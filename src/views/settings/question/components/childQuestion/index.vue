...
<script>
import { getQuestionChildrenList, editProblem, enableProblem, moveProblem, addProblem, getFloorInfo } from '@/api/operate/question'
...
export default {
  ...
  methods: {
    ...
    tableAction(dat, row, index, name) {
      ...
      getFloorInfo(param).then(res => {
        this.$refs.complaintForm.recoveryForm({
          cpProblemRemark: res.payload.cpTypeTwo
        })
      })
      switch (name) {
    ...
    addComplintQuestion(row) {
      this.rowData = row
      const { channelId, cpType } = this.row
      this.info = {
        title: '新建投诉问题',
        instroduct: '',
        type: 'addComplintQuestion'
      }
      this.complaintForm = addComplintQuestionChildrenForm(false)
      this.dialogNewVisible = true
      this.$nextTick(() => {
        this.$refs.complaintForm.recoveryForm({
          channelPid: channelId + '',
    ...
    addComplintQuestionFn(val) {
      ...
      const data = {
        ...val,
        pid: this.rowData.problemIdNew,
        cpType: val.problemIdThree,
        businessId
      }
