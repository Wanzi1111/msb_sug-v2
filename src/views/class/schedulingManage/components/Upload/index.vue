<template>
  <div class="upload-demo" style="text-align: center;">
    ...
    <ul class="el-upload-list el-upload-list--text" style="width: 360px;margin: 0 auto">
...
export default {
  ...
  methods: {
    ...
    */
    async uploadVerification(file, mediaRules = { width: 0, height: 0, ratio: 0, type: '', size: 1024 }) {
      if (!file) {
        return false
      }
    ...
    actUp(config = {}) {
      const { file, reason } = this
      const that = this
      const operatorPhone = this.$store.state.user.phone
      that.loaded = 0
      this.uploadApi({
        ...config,
        file: file,
