...
<template>
  <div class="action-container">
    <template v-for="(item, index) in actions">
      ...
      <!-- 普通操作渲染 -->
      <div v-else v-show="!item.attr.showbtn" :key="index" :class="actionItemStyle(item)">
        <!--按钮-->
        <el-button v-if="item.type === 'button'" v-permission="item.attr && item.attr.permission" v-bind="item.attr" :loading="loadings[item.attr ? item.attr.loading : false]" @click="item.click">{{ item.label }}</el-button>
        <!--a标签按钮-->
        <a v-if="item.type === 'a'" href="javascript:;" v-bind="item.attr" @click="item.click">{{ item.label }}</a>
        <!--a标签链接-->
        <a v-if="item.type === 'link'" v-bind="item.attr">{{ item.label }}</a>
  ...
  methods: {
    actionItemStyle(dat) {
      ...
      if ('align' in dat && dat.align) {
        return `${style[dat.type]} ${align[dat.align]} ${dat.noMargin ? 'no-margin-action-item' : ''}`
      }
