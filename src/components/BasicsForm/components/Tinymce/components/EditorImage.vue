    ...
    <el-dialog width="750px" title="上传图片" :visible.sync="dialogVisible">
      <div class="ed-img-list">
        <ul class="materiel-list">
          <li v-for="(item, i) in imgList" :key="i" :class="item.check ? 'img-check' : ''" @click="selectMateriel(i)">
            <img :src="item.url" alt="">
            <div :class="item.check ? 'materiel-check materiel-check-y' : 'materiel-check'">
      ...
      <div class="form-item-element">
        <input ref="img" style="display: none" type="file" @change="uploadMedia">
      ...
      <div class="ed-img-action">
        <div><el-button icon="el-icon-check" type="primary" @click="handleSubmit">确定</el-button></div>
        <div><el-button icon="el-icon-close" @click="dialogVisible = false">取消</el-button></div>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    uploadBefore(file, mediaRules = { type: '', size: 2048 }) {
      ...
      if (isType && isBig) {
        ...
        this.$message.error(`请上传${mediaRules.size}M以下文件，支持${mediaRules.type}格式`)
        return false
      }
    },
    // 点选物料
