  ...
  <div class="form-item-element" v-bind="item.attr">
    ...
    <template v-if="loadingProp.indexOf(item.prop) === -1 && url">
      <div class="media-item">
        ...
        <audios v-else-if="url.indexOf('mp3') != -1" :url="url" />
        <videos v-else :key="url" :url="url" :poster="poster" :poster-ratio="item.mediaRules.posterRatio" :auto-poster="!poster && item.mediaRules.poster" @dataUri="dataUri" />
        <template v-if="!item.attr || !item.attr.disabled">
          <div class="media-upload-update" @click="clickUpload(item.prop)"><i class="el-icon-upload2" />&ensp;修改</div>
          <div class="media-upload-delete" @click="deleteVideo()"><i class="el-icon-delete" />&ensp;删除</div>
        </template>
      </div>
    </template>
    ...
    <div v-else-if="item.drag && (!item.attr || !item.attr.disabled)" class="media-upload media-upload-drag" @dragover="fileDragover" @drop="e => fileDrop(e, item)" @click="clickUpload(item.prop)">
      拖拽文件到此处，或者<a>点击上传</a>
    </div>
    <div v-else-if="!item.attr || !item.attr.disabled" class="media-upload">
...
export default {
  ...
  methods: {
    ...
    fileDrop(e, item) {
      e.preventDefault()
      const { dataTransfer: { files }} = e // 获取到第一个上传的文件对象
      if (!files) return
      this.uploadMediaFc({ target: { files }}, item)
    },
    // 上传
