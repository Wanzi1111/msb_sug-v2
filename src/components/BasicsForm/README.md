...
forms: [{
  ...
  paste: type === upload时，为true可以支持粘贴上传
  drag: type === upload-media时，为true可以支持拖拽上传
}]
...
this.$set.form.set({
  type:'options',// *字符串 组件可选择类型及使用:  options-下拉项 defaultVal-默认值 visible-显隐 rules-校验规则 attr-属性 tinymce-富文本
  data:{ selectList: [] }, // *对象 功能及使用:  data:{ prop: 值 }
})
